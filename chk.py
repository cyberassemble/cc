...
def chk(pips, sks, byp):
    ...
    try:
        ...
        print(
            GREEN+f''' [CHARGED]  =>   {pips}   MESSAGE =>  1$  {response}  [{msg}]  BYPASSING => '''+BLUE+f'''[{byp}]'''+GREEN+'   BY - @I_M_R_A_S')
        with open("charged.txt", 'a') as sav:
            sav.write(
                f''' [CHARGED]  =>   {pips}   MESSAGE =>  1$  {response}  [{msg}]  BYPASSING => [{byp}]   BY - @I_M_R_A_S''')
            sav.write('\n')
        return True
        ...
        try:
            ...
            if respp == 'incorrect_cvc':
                print(
                    GREEN+f''' [CCN] =>   {pips}   MESSAGE =>   {msg}  [{respp}]  BYPASSINGING => '''+BLUE+f'''[{byp}]'''+GREEN+'   BY - @I_M_R_A_S')
                with open("CCN.txt", 'a') as sav:
                    sav.write(
                        f''' [CCN]  =>   {pips}   MESSAGE =>   {msg}  [{respp}]  BYPASSING => [{byp}]   BY - @I_M_R_A_S''')
                    sav.write('\n')

            elif cvccheck == 'pass' or respp == 'insufficient_funds':
                print(
                    GREEN+f''' [LIVE] =>   {pips}   MESSAGE =>   {msg}  [{respp}]  BYPASSING => '''+BLUE+f'''[{byp}]'''+GREEN+'   BY - @I_M_R_A_S')
                with open("Approvedinsuff.txt", 'a') as sav:
                    sav.write(
                        f''' [LIVE]  =>   {pips}   MESSAGE =>   {msg}  [{respp}]  BYPASSING => [{byp}]   BY - @I_M_R_A_S''')
                    sav.write('\n')
            else:
                print(
                    RED + f''' [DEAD] =>   {pips}   MESSAGE =>  {msg}  [{respp}]  BYPASSING => '''+BLUE+f'''[{byp}]'''+RED+'   BY - @I_M_R_A_S')
            return True
        except error.CardError as e:
            return False
    except error.AuthenticationError as e:
